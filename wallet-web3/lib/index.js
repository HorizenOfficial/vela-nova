...
// src/constants.ts
var CHALLENGE = "horizen";
var HKDF_SALT = new Uint8Array(0);
var HKDF_INFO = new Uint8Array(0);
...
function validateObject2(object2, fields = {}, optFields = {}) {
...
function ecdsa(Point2, hash2, ecdsaOpts = {}) {
  ...
  const hasLargeCofactor = CURVE_ORDER * _2n7 < Fp2.ORDER;
  function isBiggerThanHalfOrder(number2) {
  ...
  class Signature2 {
    ...
    static fromBytes(bytes2, format = defaultSigOpts.format) {
      ...
      let recid;
      if (format === "der") {
        const { r: r2, s: s2 } = DER2.toSig(abytes(bytes2));
  ...
  function sign(message, secretKey, opts = {}) {
    ...
    const drbg = createHmacDrbg2(hash2.outputLen, Fn.BYTES, hmac3);
...
var Fn521 = /* @__PURE__ */ (() => Field2(p521_CURVE.n, { allowedLengths: [65, 66] }))();
var p521_Point = /* @__PURE__ */ weierstrass2(p521_CURVE, { Fn: Fn521 });
var p521 = /* @__PURE__ */ ecdsa(p521_Point, sha5123);
...
function bigIntToUint8Array(n2, length) {
  const bytes2 = new Uint8Array(length);
  let temp = n2;
  for (let i = length - 1; i >= 0; i--) {
    bytes2[i] = Number(temp & BigInt(255));
    temp >>= BigInt(8);
  }
  return bytes2;
}
...
async function deriveKeyPairFromSeed(seed) {
  ...
  while (true) {
    ...
    const hashHex = bytesToHex3(hash2);
    const candidate = BigInt("0x" + hashHex);
    if (candidate > BigInt(0) && candidate < n2) {
      d = candidate;
      break;
    }
    counter++;
...
async function deriveKeyPairFromHKDF(ikm, salt, info) {
  const n2 = p521.Point.Fn.ORDER;
  let d;
  let counter = 0;
  while (true) {
    ...
    derivedBytes[0] &= 1;
    const hashHex = bytesToHex3(derivedBytes);
    const candidate = BigInt("0x" + hashHex);
    if (candidate > BigInt(0) && candidate < n2) {
      d = candidate;
      break;
...
async function exportPublicKeyToHex(publicKey) {
  const bytes2 = new Uint8Array(await crypto.subtle.exportKey("raw", publicKey));
  return bytesToHex3(bytes2);
}
async function encrypt(senderPrivateKey, receiverPublicKey, message) {
  const sharedSecret = await crypto.subtle.deriveBits(
    { name: "ECDH", public: receiverPublicKey },
    senderPrivateKey,
    528
    // P521 uses 528 bits (rounded up from 521)
  );
  const aesKey = await deriveAES256KeyFromBits(new Uint8Array(sharedSecret));
  return await encryptWithAES(aesKey, message);
}
async function decrypt2(receiverPrivateKey, senderPublicKey, ciphertext) {
  const sharedSecret = await crypto.subtle.deriveBits(
...
async function deriveAES256KeyFromBits(sharedBytes) {
...
async function deriveP521PrivateKeyFromSigner(signer, useAlternativeSign, challengeOverride, hkdfSaltOverride, hkdfInfoOverride) {
  ...
  const messageToSign = challenge + address;
  const signature = useAlternativeSign && signer.provider instanceof JsonRpcProvider ? await _alternativeSign(signer, messageToSign) : await signer.signMessage(messageToSign);
  const sigBytes = hexToBytes3(signature);
  return await deriveKeyPairFromHKDF(sigBytes, hkdfSalt, hkdfInfo);
}
async function _alternativeSign(signer, message) {
  const prefix = `Ethereum Signed Message:
${message.length}`;
  const prefixed = ethers_exports.toUtf8Bytes(prefix + message);
  const hash2 = ethers_exports.keccak256(prefixed);
  return await signer.provider.send("eth_sign", [await signer.getAddress(), hash2]);
}
async function ethersSignerFromBrowser() {
  if (!window.ethereum) {
    throw new Error("wallet not connected");
  }
  const ethereum = window.ethereum;
  const provider = new ethers_exports.BrowserProvider(ethereum);
  const signer = await provider.getSigner();
  return signer;
}

// src/typechain-types/factories/contracts/interfaces/ITeeAuthenticator__factory.ts
var _abi = [
  {
    inputs: [],
    name: "TeeIsNotSet",
...
var ITeeAuthenticator__factory = class {
...
ITeeAuthenticator__factory.abi = _abi;

// src/typechain-types/factories/contracts/ProcessorEndpoint__factory.ts
var _abi2 = [
  {
    inputs: [
      {
...
var _bytecode = "";
var isSuperArgs = (xs) => xs.length > 1;
var ProcessorEndpoint__factory = class extends ContractFactory {
  constructor(...args) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
  ...
  getDeployTransaction(_teeAuthenticator, _authorityRegistry, updateStatusOperator, admin, _minFeePerRequest, overrides) {
  ...
  deploy(_teeAuthenticator, _authorityRegistry, updateStatusOperator, admin, _minFeePerRequest, overrides) {
    return super.deploy(
      _teeAuthenticator,
      _authorityRegistry,
      updateStatusOperator,
...
ProcessorEndpoint__factory.bytecode = _bytecode;
ProcessorEndpoint__factory.abi = _abi2;

// src/blockchain/client.ts
var RequestType = /* @__PURE__ */ ((RequestType2) => {
  RequestType2[RequestType2["DEPLOYAPP"] = 0] = "DEPLOYAPP";
  RequestType2[RequestType2["PROCESS"] = 1] = "PROCESS";
  RequestType2[RequestType2["DEANONYMIZATION"] = 2] = "DEANONYMIZATION";
  RequestType2[RequestType2["ASSOCIATEKEY"] = 3] = "ASSOCIATEKEY";
  return RequestType2;
})(RequestType || {});
var RequestReceipt = class {
  constructor(requestId, transactionReceipt) {
    this.requestId = requestId;
    this.transactionReceipt = transactionReceipt;
  }
};
var RequestResult = class {
  constructor(requestId, status, errorCode, errorMessage) {
    this.requestId = requestId;
    this.status = status;
...
var HorizenCCEClient = class {
  constructor(signer, useAlternativeSign, teeAuthenticatorAddress, processorEndpointAddress) {
    this.signer = signer;
  ...
  async submitRequest(protocolVersion, applicationId, requestType, payload, depositAmount, maxFeeValue) {
  ...
  async submitRequestAndWaitForRequestId(protocolVersion, applicationId, requestType, payload, depositAmount, maxFeeValue) {
    const tx = await this.submitRequest(protocolVersion, applicationId, requestType, payload, depositAmount, maxFeeValue);
    const receipt = await tx.wait();
    const events = receipt?.logs;
    const requestId = events && events.length > 0 ? events[0].topics[1] : void 0;
    if (!requestId) {
  ...
  async getRequestCompletedEvent(requestId, fromBlock, toBlock) {
    if (fromBlock != void 0 && toBlock != void 0 && fromBlock < toBlock) {
    ...
    return new RequestResult(
      ...
      event.args.status,
      event.args.errorCode || void 0,
      event.args.errorMessage || void 0
    );
  }
  async getCurrentUserEvents(fromBlock, toBlock, applicationId, eventSubType, filter, stopAtFirst) {
    if (fromBlock != void 0 && toBlock != void 0 && fromBlock < toBlock) {
      throw new Error("fromBlock cannot be less than toBlock");
    }
    const events = await this.processorEndpoint.queryFilter(
      this.processorEndpoint.filters.UserEvent(applicationId, void 0, eventSubType, void 0),
      toBlock,
      fromBlock
    );
    return await this.decryptAndFilterEvents(events, filter, stopAtFirst);
  }
  async decryptAndFilterEvents(events, filter, stopAtFirst) {
    const teePublicKeyString = await this.getTeePublicKey();
    const teePublicKey = await importPublicKeyFromHex(teePublicKeyString);
    const privateKey = (await this.getSignerKeyPair()).privateKey;
    const returnEvents = [];
    for (let i = events.length - 1; i >= 0; i--) {
      try {
        const encryptedData = hexToBytes3(events[i].args.encryptedData);
        const decryptedData = await decrypt2(privateKey, teePublicKey, encryptedData);
